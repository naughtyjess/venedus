...
public class UnpunishCommand extends Command {
    ...
    public UnpunishCommand() {
        super("unpunish", null, "unban", "pardon");
    }
            ...
            if (args.length == 2) {
                ...
                if (target != null) {
                    if (target != player) {
                        ...
                        if (type != null) {
                            OriginPunishment punishment = target.getActivePunishments().stream().filter(originPunishment -> originPunishment.getType() == type).findFirst().orElse(null);
                            if (punishment != null && punishment.isActive()) {
                                PunishmentManager.getInstance().executeUnpunish(punishment.getId(), punishment.getTarget(), punishment.getType(), new Timestamp(new Date().getTime()));
                                OriginManager.getInstance().getPlayers().forEach(players -> {
                                    if (players.hasPriorityAccess(Rank.SUPPORTER.getPriority()) && NotifyManager.getInstance().getObject(players.getUniqueId()).get("ban").getAsBoolean()) {
                                        players.sendMessage();
                                        players.sendMessage(
                                                Origin.getInstance().getPrefix() + target.getDisplayName() + "'s §7Ban wurde von " + player.getDisplayName() + " §7aufgehoben.",
                                                Origin.getInstance().getPrefix() + target.getDisplayName() + "'s §7ban was removed by " + player.getDisplayName() + "§7."
                                        );
                                        players.sendMessage(Origin.getInstance().getPrefix() + "§a[" + player.language("KEIN GRUND ANGEGEBEN", "NO REASON DEFINED") + "]");
                                        players.sendMessage();
                                    }
                                });
                            } else {
                    ...
                    if (target != player) {
                        ...
                        if (type != null) {
                            ...
                            if (punishment != null && punishment.isActive()) {
                                ...
                                IntStream.range(2, args.length).forEach(value -> reason.append(value == 2 ? "§a" : " §a").append(args[value]));
                                ...
                                OriginManager.getInstance().getPlayers().forEach(players -> {
                                    if (players.hasPriorityAccess(Rank.SUPPORTER.getPriority()) && NotifyManager.getInstance().getObject(players.getUniqueId()).get("ban").getAsBoolean()) {
                                        ...
                                        players.sendMessage(Origin.getInstance().getPrefix() + "§a[" + reason.toString().toUpperCase() + "]");
                                        players.sendMessage();
                                    }
                                });
                            } else {
                ...
                player.sendMessage(
                        Origin.getInstance().getPrefix() + "§7Bitte nutze: §f/unpunish [mute/ban] [Name]",
                        Origin.getInstance().getPrefix() + "§7Please use: §f/unpunish [mute/ban] [Name]"
                );
                player.sendMessage(
                        Origin.getInstance().getPrefix() + "§7Bitte nutze: §f/unpunish [mute/ban] [Name] [Grund]",
                        Origin.getInstance().getPrefix() + "§7Please use: §f/unpunish [mute/ban] [Name] [Rason]"
                );
            }
        } else {
            player.sendMessage(
